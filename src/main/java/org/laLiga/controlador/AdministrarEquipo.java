...
public class AdministrarEquipo implements Crud {
    ...
    public Equipo estadisticas(String atributoRequerido, List<Equipo> listaEquipos){
        Equipo resultado = null;

        if (atributoRequerido.equals("goles")){
            for (Equipo equipos : listaEquipos) {
                if (resultado == null || equipos.getGf() > resultado.getGf()) {
                    resultado = equipos;
                }
            }
        }else if(atributoRequerido.equals("puntos")){
            for (Equipo equipos : listaEquipos) {
                if (resultado == null || equipos.getTp() > resultado.getTp()) {
                    resultado = equipos;
                }
            }
        }else if(atributoRequerido.equals("partidos")){
            for (Equipo equipos : listaEquipos) {
